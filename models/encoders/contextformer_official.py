"""Vendored, dependency-free copy of the official Contextformer architecture.

Source: earthnet-models-pytorch @ v0.1.0
  earthnet_models_pytorch/model/contextformer.py
  (Benson et al., "Multi-modal Learning for Geospatial Vegetation
   Forecasting", CVPR 2024 — the GreenEarthNet SOTA)

Why vendored here (plan-b-pvt only):
  * The official package pins torch 1.13.1 / pytorch-lightning 1.7.7, which
    will not run on our H200 (sm_90) or local Blackwell (sm_120) GPUs.
  * The *model* itself is pure torch + timm and runs fine on torch 2.x.
  * We therefore copy ONLY the nn.Module (no Lightning / no emp package), so
    Gate-0 A2 (reproduce Contextformer in our modern stack + load the official
    weights) and B0 (matched fine-tune) can run on our GPUs.

Faithfulness notes (the forward pass is load-bearing for evaluator parity):
  * `Attention`, `Mlp`, `LayerScale`, `Block`, `get_sinusoid_encoding_table`,
    `PVT_embed`, `ContextFormer.forward` are reproduced VERBATIM.
  * The only deviations from the upstream file:
      - the emp import `inverse_permutation` is inlined below (trivial);
      - `add_model_specific_args` (argparse / emp `str2bool`) is dropped — we
        build hparams directly (see pvt_contextformer_q.py);
      - `PVT_embed` `pretrained` is taken from `hparams.pvt_pretrained`
        (default True) so the weight-loading path can skip the timm download.
  * Official state_dict keys are prefixed `model.` (LightningModule wraps
    `self.model = ContextFormer(...)`); strip that prefix before loading here.
"""
...
def inverse_permutation(perm):
    # inlined from earthnet_models_pytorch.model.layer_utils
    ...
    def __init__(self, hparams):
        ...
        if self.hparams.pvt:
            self.embed_images = PVT_embed(
                ...
                pretrained=getattr(self.hparams, "pvt_pretrained", True),
