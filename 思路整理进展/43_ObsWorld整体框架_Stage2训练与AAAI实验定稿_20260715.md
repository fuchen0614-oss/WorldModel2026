...
> **2026-07-16 协议仲裁更新。**本文件给出当前方法和论文叙事总览。数据目录、最终 evaluator（评估器）和主/补充测试轨道以 [`49_ObsWorld_EarthNet2021x与GreenEarthNet数据协议审计问答_20260716.md`](49_ObsWorld_EarthNet2021x与GreenEarthNet数据协议审计问答_20260716.md) 的审计闸门为准；48 仅保留为 raw EarthNet2021x 数据契约的历史方案。DGH 与实验层次以 44、45、46、47 为补充。
...
4. `observable verification`（可观测核验）：最终预测回到未来 RGBN，并由审计后冻结的官方评估器检验；ENS 与 GreenEarthNet evaluator 不混用。
...
- 若走原始 EarthNet2021 ENS 分支，`iid/ood` 是主表，`extreme/seasonal` 是补充；
- 若走 GreenEarthNet CVPR-2024 分支，则需要独立通过 `val_chopped/ood-t_chopped` 的审计，主表使用其官方 evaluator；
- 两条分支使用相同的 Stage2 训练根和 DGH 路径，但不能混合数字、mask 或测试集选择规则。
