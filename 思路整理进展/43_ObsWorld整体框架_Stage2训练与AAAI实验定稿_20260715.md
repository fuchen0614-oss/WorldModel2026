...
> **2026-07-16 修订版。**本文件给出当前方法和论文叙事总览。数据目录、manifest（文件清单）和服务器命令以 [`48_ObsWorld_EarthNet2021x统一数据协议与主实验规范_20260716.md`](48_ObsWorld_EarthNet2021x统一数据协议与主实验规范_20260716.md) 为准；DGH 与实验层次以 44、45、46、47 为补充。
...
4. `observable verification`（可观测核验）：最终预测回到未来 RGBN，并由 EarthNetScore 检验。
...
本项目只使用服务器已有的 EarthNet2021x NetCDF 数据：
...
从 `train` 中确定性划分 `train_dev` 与 `val_dev`：

- `train_dev`：模型训练、统计量计算；
- `val_dev`：选 checkpoint、调学习率、冻结训练步数；
- `iid/ood/extreme/seasonal`：任何情况下都不用于调参。

主表不是“训练集上比精度”，而是固定 IID/OOD 上的 EarthNetScore。Extreme 和 Seasonal 是支撑“受驱动长期地表演化”的补充证据，不是可有可无的附属实验。
