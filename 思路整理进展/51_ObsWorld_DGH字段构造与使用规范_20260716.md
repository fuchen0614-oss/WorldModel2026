...
> [!warning] 与 42 号文档的关系
> 42 中“DOY + 降水 + 温度 + VPD + 辐射”的 6-D 设计保留了重要的生态物理直觉，但它不是当前正式代码的最终维度。当前主路径已经依据 raw EarthNet2021x 的 8 个 E-OBS 字段升级为 **24-D**，并把 DOY 从 D 中拆成独立的 **C**。原设计最接近的四个天气变量保留为 `D_core12` 消融；旧 9-D 累计版本保留为 legacy（历史）基线。
