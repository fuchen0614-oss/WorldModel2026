# ObsWorld：EarthNet2021x 统一数据协议、清单冻结与主实验规范

> **状态：当前唯一的数据与评测口径。**本文件替代 43–47 中所有与数据集名称、split（数据划分）、验证集、测试集和主指标有关的旧约定。Stage1/Stage1.5、DGH、`phi` 和世界模型三段式叙事仍保留；改变的只是把实验严格落在服务器已经拥有的数据上。

## 0. 一句话结论

我们使用服务器已有的 **EarthNet2021x NetCDF 数据发布**，并采用 **EarthNet2021 标准评测协议**：
...
不换数据集、不重新下载、不把不存在于本地数据中的细分轨道写入论文。
...
该结构正对应 EarthNet2021 的四条测试轨道。`EarthNet2021x` 仅说明文件采用 NetCDF 格式并且包含当前可用的扩展字段；它不改变本项目应采用的实验名称和评测规则。
