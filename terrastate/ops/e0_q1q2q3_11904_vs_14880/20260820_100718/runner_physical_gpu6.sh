#!/usr/bin/env bash
# 物理 GPU 6: 第一波 gpu3_legacy11904_val_q1q2, 第二波 gpu5_legacy11904_oodt_q3
set -euo pipefail
...
LAUNCH_REC="$RETRY_DIR/launch_record_shard_pgpu6.json"
echo '{"physical_gpu":6,"jobs":[]}' > "$LAUNCH_REC"

function log_event() {
    echo "[$(date -u +%Y-%m-%dT%H:%M:%S.%6N+00:00)] $*" | tee -a "$RETRY_DIR/runner_pgpu6.log"
}
...
log_event "========== 物理GPU 6 开始 =========="

# 第一波: gpu3_legacy11904_val_q1q2
run_q1q2 "gpu3_legacy11904_val_q1q2" \
...
# 第二波: gpu5_legacy11904_oodt_q3
run_q3 "gpu5_legacy11904_oodt_q3" \
    "644deaac0b1578cd153eaffb65bddd6c5ac55d30e0bc09b595111588471e1acd" \
    "/csy-mix02/cog8/zjliu17/Agent/model-artifacts/objects/sha256/64/644deaac0b1578cd153eaffb65bddd6c5ac55d30e0bc09b595111588471e1acd.pt"

log_event "========== 物理GPU 6 完成 =========="
