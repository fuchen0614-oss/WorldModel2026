#!/usr/bin/env bash
# GPU 2 runner: v14880 三项队列（oodt_q1q2 → val_q1q2 → oodt_q3）
set -euo pipefail

RETRY_DIR="/csy-mix02/cog8/zjliu17/Agent/WorldModel2026v2/terrastate/ops/e0_q1q2q3_11904_vs_14880/20260820_100718"
MANIFEST="/csy-mix02/cog8/zjliu17/Agent/WorldModel2026v2/terrastate/ops/e0_q1q2q3_11904_vs_14880/20260818_154859/launch_manifest.json"
...
LAUNCH_REC="$RETRY_DIR/e0_launch_record.gpu2.json"
echo '{"schema":"e0_launch_record_v1","gpu":2,"jobs":[]}' > "$LAUNCH_REC"

function log_event() {
    echo "[$(date -u +%Y-%m-%dT%H:%M:%S.%6N+00:00)] $*" | tee -a "$RETRY_DIR/e0_runner_gpu2.log"
}
...
function check_gpu_free() {
    ...
    # 放宽阈值：允许自己的首次加载内存（< 20 GB）和低利用率（< 10%）
    if [ "$procs" -gt 1 ] || [ "$mem" -gt 20480 ] || [ "$util" -gt 10 ]; then
...
EOF
    ...
    $PY -c "import json,sys; r=json.load(open('$LAUNCH_REC')); r['jobs'].append($entry); json.dump(r,open('$LAUNCH_REC','w'),indent=1)"
...
log_event "========== GPU 2 queue start =========="

# job 1: gpu1_v14880_oodt_q1q2 (原 gpu1，现在改成 gpu2)
run_job "gpu2_v14880_oodt_q1q2" \
...
# job 2: gpu0_v14880_val_q1q2 (原 gpu0，现在改成 gpu2)
run_job "gpu2_v14880_val_q1q2" \
    "a5d2a0cc28ad7c01c7e314fd1e02ceb5022e1a9c5733870ebe89c490a594e94f" \
    "/csy-mix02/cog8/zjliu17/Agent/model-artifacts/objects/sha256/a5/a5d2a0cc28ad7c01c7e314fd1e02ceb5022e1a9c5733870ebe89c490a594e94f.pt" \
    "q1q2" \
    "val" \
...
# job 3: gpu2_v14880_oodt_q3 (原 gpu2，现在也是 gpu2)
run_q3_job "gpu2_v14880_oodt_q3" \
    "a5d2a0cc28ad7c01c7e314fd1e02ceb5022e1a9c5733870ebe89c490a594e94f" \
    "/csy-mix02/cog8/zjliu17/Agent/model-artifacts/objects/sha256/a5/a5d2a0cc28ad7c01c7e314fd1e02ceb5022e1a9c5733870ebe89c490a594e94f.pt"

log_event "========== GPU 2 queue done =========="
