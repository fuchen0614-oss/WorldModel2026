#!/usr/bin/env bash
# 物理 GPU 2: 运行 gpu1_v14880_oodt_q1q2 (任务名按冻结清单，物理卡是2)
set -euo pipefail
...
LAUNCH_REC="$RETRY_DIR/launch_record_shard_pgpu2.json"
echo '{"physical_gpu":2,"jobs":[]}' > "$LAUNCH_REC"

function log_event() {
    echo "[$(date -u +%Y-%m-%dT%H:%M:%S.%6N+00:00)] $*" | tee -a "$RETRY_DIR/runner_pgpu2.log"
}

log_event "========== 物理GPU 2 开始 =========="

# 任务1: gpu1_v14880_oodt_q1q2 (冻结清单中的名字)
job_name="gpu1_v14880_oodt_q1q2"
