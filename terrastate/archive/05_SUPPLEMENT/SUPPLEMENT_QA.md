...
| D. Q3 Heat--Drought Subset and Weather-Control Protocol | Define the frozen extreme-weather selection, matched controls, and evidential boundary. | Q80 training-climatology thresholds; weather coverage; excluded selection signals; 84 pairs, 45 donors, 31 clusters; cluster bootstrap; non-causal scope. | PASS |
...
| Q3 validity | Is the extreme-weather subset frozen independently of outcomes and are donor dependence and scope handled? | Appendix D. | PASS |
...
- Exactly one `booktabs` table remains, with its caption above the table and
  no vertical rules.
...
| Objective | ground truth + 0.5 KD + scheduled future-state term | training entry point | PASS |
