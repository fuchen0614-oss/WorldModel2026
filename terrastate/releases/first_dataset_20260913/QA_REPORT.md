...
### 第二轮追加：图注/表注下的"用途与效果"段落

同一轮之后又做了一次纯增补：两份主文里每一张图、每一个表的图注/表注下方各加一段
`> **用途与效果｜** …`（01 共 17 处、02 共 11 处），并同步到 9 个图目录的 `CAPTION_ZH.md`、
`reports/FIGURE_CAPTIONS_ZH.md` 与 `OVERVIEW.md`（11 个文件）。段落只复述本包已冻结的
判据与数字，未新增统计、未改任何图表与数值。相应文件已登记进 `provenance/RELEASE_EDITS.csv` 并刷新哈希。
