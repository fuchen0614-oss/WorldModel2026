...
- **当前状态（2026-08-24 锁定集终局补记）：`Q4_LOCKED_COMPLETE_QUALIFIED_FAIL_NO_RERUN`** —— 固定的
  4 卡 C1/C0R pair 已各跑满 14,880 步，并已完成唯一一次 `val_locked` Q4。C1 单臂四门 PASS，
  但 C1 对 C0R 的事实端点 `G_abs` 为 4/19、overall FAIL；不得把它写成 Q4 整体通过或事实非劣。
...
**这道门在问什么，为什么它 FAIL 不推翻 §15.3**：
`G_abs` 问的是"C1 相对 C0R **在事实端点上没有变差**"。它是一道**副作用门**，
用来防止"组合性变好是靠牺牲事实精度换来的"。它 FAIL 说明：
在 ep15/ep20 上，C1 的事实端点精度**不能被证明不比 C0R 差**（与 §15.1 的 val 数字方向一致——
C0R 的端点 val 确实略低）。

所以当前可支持与不可支持的表述必须分开写：

- **可以说**：C1 单臂通过 Q4 四道门；C0R 不通过；C1 的分段退化（0.9%–1.4%）
  显著小于 C0R（6.4%–15.1%）；C1 的状态在 20 步内不塌缩。
- **不能说**：C1 在事实预测上不劣于 C0R。这是 `G_abs` 要证的，它没过。
- **不能说**：Q4 整体 PASS。整体 verdict 是 FAIL，因为整体 verdict 含 `G_abs`。
- **准确的一句话**：*recursive 臂获得了组合一致性，但在较远端点上无法证明事实精度不劣化。*
...
### 17.3 C1 对 C0R 的事实端点门：**FAIL，4/19**
...
的所有组合均通过。本次只有 **4/19** 通过：direct **1/3**，composed **3/16**，因此
`direct_all_pass=false`、`composed_all_pass=false`、overall verdict **FAIL**。
...
这组结果说明该固定 pair 上 C1 的**分段稳定性信号**显著强于 C0R（0.8%–1.2% 对
9.1%–14.7% 的最差分段退化），但不满足“C1 在所有事实端点不劣于 C0R”的合同。因而：

- **可以说**：在这个固定 4 卡内部配对和锁定 split 上，recursive C1 单臂通过四个 Q4 门，而 direct
  C0R 未通过；C1 在分段预测下表现出更强的稳定性。
- **不能说**：Q4 overall PASS、事实预测非劣、可组合预测状态已经被完整证明，或 C2/C3/C0S/C4/C5
  已获得任何证据。

### 17.4 资格口径、资格限定与下一步边界

主口径仍为 `n_valid≥64`，约排除 44.7% 的 `(cube, combo)` 对；它**不是预注册的**且会改变结论，
所以不能隐藏。并列敏感性下的臂间通过数为：none（仅 `sst>0`）**1/19**、std-v1（`std≥1e-2`）
**5/19**、主口径 **4/19**。三者均未通过总门，不存在可被选择出来的“成功口径”。
...
interval=372 的启动合同。该偏离和后验资格线必须同结果一并披露。

**终局状态：`Q4_LOCKED_COMPLETE_QUALIFIED_FAIL_NO_RERUN`。** 锁定集不再读取、不重跑、不换 pair、不改
资格线或 bootstrap。后续若要进入 C2/C3，必须先另行冻结新的开发集驱动协议与损失选择规则，且不得用本次
`val_locked` 数字调 λ、选 checkpoint 或决定重跑；C0S/C4/C5 仍受正式 simulator 数据、mapping、scenario
manifest 缺失的硬阻塞。
...
> **§17.3 的 `Q4_LOCKED_COMPLETE_QUALIFIED_FAIL_NO_RERUN` 判决原样有效，`val_locked` 仍然封存。**
