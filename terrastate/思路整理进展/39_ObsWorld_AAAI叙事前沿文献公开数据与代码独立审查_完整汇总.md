...
> **协议状态（2026-07-16）：**本篇保留独立审查中的方法风险和实验思想，但不再是数据协议或代码实现的入口。当前唯一执行口径是服务器已有的 EarthNet2021x NetCDF，加 EarthNet2021 `train/iid/ood/extreme/seasonal` 协议；清单冻结、开发验证、主表与指标请直接以 [48：统一数据协议](48_ObsWorld_EarthNet2021x统一数据协议与主实验规范_20260716.md) 和 47 为准。
...
6. 统一 EarthNet2021x 官方数据身份和官方指标协议。
...
~~~
EarthNet2021x / EarthNet 历史多光谱观测
    ↓
Stage 1.5 编码器
    ↓
历史状态 token
...
- [EarthNet2021 原论文](https://arxiv.org/abs/2104.10066)

本项目将服务器目录中的 `earthnet2021x` 视为 EarthNet2021x NetCDF 数据发布；它不是需要另行下载的第二套数据。本项目的测试轨道和评分以 EarthNet2021 协议为准，而不是从目录名推导额外细分轨道。

建议：

- 主动力学数据统一称 EarthNet2021x NetCDF；
- 主表只报告冻结的 IID/OOD 清单，补充表报告 Extreme/Seasonal；
- 主指标使用 EarthNetScore（ENS）及 MAD/OLS/EMD/SSIM 分量；
- 任何额外误差或 NDVI 诊断均单列为支持证据，不能替代主表。
...
5. 验证主指标仍偏向 MAE  
   当前配置的 primary metric 不是 EarthNet2021x 官方核心指标，也不是机制/OOD 指标。保存最佳模型时应同时监测官方主指标与状态/组合验证指标，避免只优化像素平均误差。
...
关键约束：这些实验必须共享同一套 state interface（状态接口）和尽量相同的最终编码器/checkpoint。若在 SSL4EO 和 EarthNet2021x 上训练两个完全不同、互不相连的模型，它们不能共同证明一个统一世界模型。Stage 2 后必须重新验证 Stage 1.5 的状态性质，或采用 frozen/EMA encoder 和 SSL4EO replay。
...
| EarthNet2021x | 主动力学、驱动条件预测、组合性、时空 OOD | 外生驱动条件下的植被状态/观测预测 | 不能证明天气因果；主要是欧洲生长季 |
...
- [EarthNet2021 官方模型实现](https://github.com/earthnet2021/earthnet-models-pytorch)
...
可作为旧基准兼容，但不建议继续作为主数据身份。旧 ENS/EarthNetScore 可以放补充材料，主文应切到 EarthNet2021x 的官方协议。还需明确：

- `earthnet2021x` 是服务器上当前 NetCDF 数据目录的名称；论文只将其表述为 EarthNet2021x NetCDF 数据发布；
...
数据：EarthNet2021x，按 EarthNet2021 的 IID、OOD、Extreme、Seasonal 冻结清单评测。
...
第一层：EarthNet2021x IID、OOD、Extreme、Seasonal。
...
- OOD/Extreme/Seasonal 不应只在一个划分偶然提升；
- 主结果至少 3 个随机种子。
...
4. 实验：L1C/L2A 状态分工、EarthNet2021x 官方主表、组合性、驱动对照、OOD/极端；
...
| EarthNet2021x 不及强基线 | 官方主表 | 可容忍有限差距 | 若状态/组合/OOD 强，可定位机制论文；差距过大则先提升预测 |
...
EarthNet2021x 的 IID/OOD 主表与 Extreme/Seasonal 补充表，报告 ENS 及分量，并列清输入数据和未来天气是否可用。
...
| EarthNet2021x 数据读取 | D:\Mine Program\codexwork\CVPRAAAI投稿\WorldModel2026\data\datasets\earthnet2021.py |
...
5. 使用 SSL4EO + EarthNet2021x 形成最小闭环，DeepExtremeCubes 作为最高价值外部数据；
