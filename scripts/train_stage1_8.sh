#!/usr/bin/env bash
# plan-b-pvt · Stage1.8 phi-factorization launcher (8-GPU DDP).
#   conda activate WorldModel
#   CACHE_DIR=/tmp/zjliu17_l1c_l2a_cache OUTPUT_DIR=checkpoints/plan_b_stage1_8 \
#     bash scripts/train_stage1_8.sh
set -euo pipefail
GPUS="${GPUS:-8}"
...
mkdir -p "$OUTPUT_DIR"
echo "GPUS=$GPUS CACHE_DIR=$CACHE_DIR OUTPUT_DIR=$OUTPUT_DIR PER_GPU_BATCH=$PER_GPU_BATCH MAX_EPOCHS=$MAX_EPOCHS"
