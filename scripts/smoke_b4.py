...
def main():
    ...
    # 4. lambdas.dyn>0 -> non-trivial finite latent-future; preds unchanged
    preds1, aux1 = model(data, pred_start=cl, preds_length=tl, lambdas=SimpleNamespace(dyn=1.0))
    lf = float(aux1["logs"]["latent_future"].item())
    ok_dyn = torch.isfinite(aux1["total"]).item() and lf > 0.0
    checks.append(("lambdas.dyn=1 -> latent_future>0 & finite", ok_dyn, f"latent_future={lf:.4e}"))
    with torch.no_grad():
    ...
    # gradient actually flows into the transition (real dynamics lever). NB: the
    # transition's last layer is zero-init (identity start), which blocks grad to
    # earlier layers on step 0 — so check the WHOLE module, not net[0].
    aux1["total"].backward()
    grads = [p.grad for p in model.transition.parameters() if p.grad is not None]
    gsum = sum(g.abs().sum().item() for g in grads)
    ok_grad = len(grads) > 0 and all(torch.isfinite(g).all() for g in grads) and gsum > 0
    checks.append(("dyn loss has gradient into transition", ok_grad,
                   f"|grad|sum={gsum:.3e} over {len(grads)} tensors"))
