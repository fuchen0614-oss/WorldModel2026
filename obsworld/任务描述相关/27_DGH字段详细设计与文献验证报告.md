...
D 在训练和下游中扮演**完全不同的角色**，这一分工有权威文献背书：

| 阶段 | D 的角色 | 文献依据 |
|---|---|---|
| Stage 2 主实验 | 动力学的**条件输入**，做消融和反事实 | TFT 的 known-future covariate；Dreamer 的 action |
| Stage 4 下游 | **退场**，只用冻结的状态表征 z | JEPA / Presto / Galileo / SatMAE 范式 |

> [!important] 这解决了"数据泄露"疑虑
> D（未来气象）作为**已知未来输入**喂给动力学，不是泄露——就像 Dreamer 用未来 action。
> 真正的泄露是用未来的**观测/植被**去预测植被。TFT（Lim et al. 2021）为此提供了正式术语框架。
> 详见文末 [[#附录B-D的角色文献链]]。
...
> Horizon-Aware GNN（arXiv 2026）证明：多跨度联合训练使长期预测 RMSE **降低 63%**。
> 模型学会"直接跳 30 天"，而非"走 30 次 1 天"，大幅减少自回归深度。
...
3. 提取 D = [doy, precip, temp, VPD, rad]，覆盖 t 到 t+h 每一步
4. 提取 G = elevation（静态）
5. 多跨度动力学预测：
   for h in [10,20,30,60]:
...
> [!note] 为什么用 FiLM 而非简单拼接
> Benson et al.(CVPR 2024) 附录 B 的消融证明：FiLM > 拼接，交叉注意力配潜融合最好。
> DiffObs（2024）证明简单拼接地理/气象信息会失败。
> FiLM 是可控性与实现简洁的平衡，且你有 codec 的 FiLM 经验。详见 [[#附录D-注入方式文献]]。
...
| 可解释性 | 弱（黑箱生成） | 强（DGH 消融 + 反事实） |
| 参数量 | 大 | 小 |

> [!important] EO-WM 只作为 SOTA 参照，非主要对手
> EO-WM 本质是扩散模型迁移到遥感 + 换一层气象叙事，其方法思路不值得照搬。
> 主要架构对标是 Earthformer（时空 transformer）；EO-WM 用于展示"物理驱动 vs 扩散生成"的路线差异。
...
| h 设计 | 多跨度联合 | Horizon-Aware GNN 证明减少累积误差 63% |
| 对标策略 | Earthformer 主要，EO-WM 参照 | EO-WM 是扩散迁移，不值得照搬；强调路线差异 |
...
- **Benson et al. CVPR 2024** — 附录 B 消融 CAT/FiLM/xAttn × early/latent 融合，结论 FiLM > 拼接。https://arxiv.org/abs/2303.16198
- **DiffObs, 2024** — 证明简单拼接地理/气象信息不足以表征动力学。https://arxiv.org/html/2404.06517v1
- **Horizon-Aware GNN, 2026** — 多跨度联合训练降低长期误差 63%。
