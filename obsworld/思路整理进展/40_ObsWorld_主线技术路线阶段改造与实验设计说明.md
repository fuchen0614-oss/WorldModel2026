...
> **数据协议更新（2026-07-16）：**本篇的方法讨论保留；所有数据划分、主表和实现应改按服务器已有的 EarthNet2021x NetCDF 与 EarthNet2021 `train/iid/ood/extreme/seasonal`，具体以 [48：统一数据协议](48_ObsWorld_EarthNet2021x统一数据协议与主实验规范_20260716.md) 为准。
...
1. 当前 EarthNet2021x 主训练接口没有经过统一验证的逐帧完整几何元数据；
...
| ENS 只是 legacy，不能用 | 原 EarthNet2021 仍应按官方 ENS 协议；EarthNet2021x 使用自身官方指标。问题是不能混用 |
...
- 固定 EarthNet2021x 与 EarthNet2021 各自官方 split 和 metric；
- 对每个消融配置做最小预跑。
...
- 数据：服务器已有的 EarthNet2021x NetCDF，按 EarthNet2021 的 IID、OOD、Extreme、Seasonal 轨道；主表完全遵循 ENS 协议；
...
| 数据 | 最适合承担 | 不应承担 |
|---|---|---|
| EarthNet2021x NetCDF | 植被预测、空间 OOD、隐藏帧重演、天气消融 | 精确几何算子、因果反事实 |
| EarthNet2021 标准协议 | 历史方法与 EO-WM 的可比评测 | 与其他指标体系混用 |
