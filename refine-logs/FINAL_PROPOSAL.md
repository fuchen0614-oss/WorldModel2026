...
- **Constraints**: 现有 Stage1/1.5 ViT 与 raw EarthNet2021x NetCDF 代码可复用；Stage1.5 正在训练、其最终 checkpoint 尚未冻结；主目标是 AAAI 级、7 页内可闭环的工作。当前主协议固定为 `train_dev/val_dev` 开发、IID/OOD 主表、Extreme/Seasonal 压力测试，不能混入 GreenEarthNet `*_chopped` 轨道。
- **Success condition**: 在冻结的 EarthNet2021 standard IID/OOD 协议上，shared-step latent model 与 matched Direct24 至少在 EarthNetScore（ENS）上统计不劣，并在时间可组合性、驱动对齐或（若实现并通过闸门）再观测校正中给出 direct/pixel-autoregressive 模型不具备的稳定收益；天气控制证明收益不是纯 calendar shortcut。
...
Contextformer 已覆盖相近的 GreenEarthNet direct forecast；PredRNN 与 UniTS v2 已覆盖 recurrent/autoregressive forecast；EO-WM 已覆盖 weather-driven partially observed EO generation；RSSM/Kalman-style learned filters 已覆盖 prior/posterior 一般范式。因此，world-model 是问题定义和系统身份，而不能单独作为首创新颖性；recurrence、weather conditioning 或 generic filtering 也不是本稿的新意。除非外部方法能在相同 raw EarthNet2021x manifest 和 ENS 导出协议下复现，否则不能将其跨协议数字放进主表。
...
3. **Evidence**: 同时验证 world model 的两项定义性能力——EarthNet2021 standard IID/OOD 的 100-day open-loop rollout，以及相对 matched generic filtering / pixel-recurrent assimilation 的 paired post-observation rollout gain（后者只有在 update 分支实现后才进入主结果）。
...
开发只用 `train_dev/val_dev`；配置锁定后在 IID 和 OOD 上各评分一次，Extreme/Seasonal 只作补充压力轨道。主表最少包含 persistence、climatology、matched Direct24、final rollout/partition ObsWorld；Contextformer、PredRNN 或 UniTS 只有按同一 raw manifest、NPZ 导出和 EarthNetScore 流程重跑时才进入数字表。

Primary official endpoint 是 EarthNetScore（ENS，越高越好）。IID/OOD 前锁定：

```text
Delta_open = ENS_Direct - ENS_ObsWorld
delta_NI = 0.01 ENS
```

paired geographic-tile bootstrap 的 one-sided upper 95% CI 必须 `<0.01`；即 Direct 相对 ObsWorld 的劣势上界不得超过 0.01 ENS。MAD/OLS/EMD/SSIM 是官方分量；同一 valid mask 上的 RGBN/NDVI MAE、horizon-wise curve 是解释性次指标，不假装成官方排名。`val_dev` 锁定后，IID/OOD 各只评一次；bootstrap replicate 必须在重采样 tiles 上重新计算官方聚合指标。
...
只有 `val_dev` 显示有益的部件才保留。Stage1 vs repaired Stage1.5 initialization 是 decision-gated Appendix row。Hard replacement、multi-reveal、EO-WM benchmark、额外数据集、downstream 与 FM 全部 cut。
...
1. Block A 在 OOD 上通过 official ENS non-inferiority，且 IID 不发生事实性崩溃；
...
2. official NPZ `highresdynamic` export、prediction manifest 和 score provenance；
3. full-20 EarthNetScore 与 official mask parity；
...
7. 32–128 cube overfit；外部 Contextformer/PredRNN 只有同协议可复现时才要求 parity；
8. one-seed proposed vs Vanilla/PredRNN-online gate 后才跑三种子。

## Downstream, Second Dataset and Large Models

当前不需要无关 downstream。固定 all-cube re-observation 是距离中心 claim 最近的能力测试。IID/OOD 已提供主表，Extreme/Seasonal 提供压力证据；第二数据集能增强外部有效性但不是七页单一主张的必要条件。
